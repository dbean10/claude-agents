...
2. **When a single LLM call is producing uneven quality across sub-tasks, decompose.** Parallel calls with narrower prompts beat one mega-prompt every time. Cost grows linearly with calls; quality grows more than linearly because each sub-prompt is sharper.
3. **Token budgets beat hard limits.** When selecting context to send to an LLM (files, chunks, messages), rank deterministically and truncate by token budget. Hard file/line/char counts produce wrong results on edge cases.
4. **Prompt-cache the static portion.** Anything that does not change between calls — system prompts, tool definitions, examples — belongs in a cached prefix.
5. **Streaming is a feature, not a transport detail.** Server-Sent Events on long-running AI work turns waiting into watching-it-think. Choose SSE for any user-facing operation that takes >5 seconds — but if the operation is async-job-shaped (queued, polled, resumable), polling is the right shape, not SSE.
6. **Tests catch real bugs.** Write tests that would have caught the last bug you wrote, not tests that just exercise the happy path.
...
3. **Write the code.** For new pure functions, write the test first. For changes to existing code, match existing patterns and run the existing tests after.
